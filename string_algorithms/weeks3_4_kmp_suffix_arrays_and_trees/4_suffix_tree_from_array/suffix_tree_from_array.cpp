...
int main() {
  ...
  map<int, vector<suffix_tree_from_array::Edge> > tree = suffix_tree_from_array::SuffixTreeFromSuffixArray(suffix_array, lcp_array, text);
  printf("%s\n", buffer);
  ...
  while (!stack.empty()) {
    ...
    const vector<suffix_tree_from_array::Edge>& edges = tree[node];
